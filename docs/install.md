...
If a workspace path is symlinked, pass an explicit `FPF_UPDATE_STATE_DIR` from the launcher or hook instead of relying on `$PWD/.fpf-update`. This keeps diagnostics and migration notes stable even when the shell reports the physical target path.
