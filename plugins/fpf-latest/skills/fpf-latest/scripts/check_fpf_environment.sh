...
PROTOCOLS_CACHE_DIR="${FPF_PROTOCOLS_CACHE_DIR:-$DEFAULT_CACHE_HOME/codex-skills-and-protocols}"
...
Checks whether the current shell environment can run fpf-latest.
...
if [ "$PORTABLE_CHECK" -eq 1 ]; then
  ...
  case "$OS_NAME" in
    ...
    MINGW*|MSYS*|CYGWIN*)
      ...
      PORTABLE_SUMMARY="Git Bash/MSYS/Cygwin-style execution is best effort; native PowerShell is not supported by these scripts."
      PORTABLE_ACTION="Use WSL or Git Bash with Bash, Git, and Unix utilities, or wait for a native PowerShell implementation."
    ...
    *)
      ...
      PORTABLE_SUMMARY="The platform is not part of the primary fpf-latest support contract."
  ...
  if [ -n "$missing_skill_files" ]; then
    ...
    PORTABLE_SUMMARY="The fpf-latest skill installation is incomplete: $missing_skill_files."
    PORTABLE_ACTION="Reinstall the whole fpf-latest directory, including SKILL.md and scripts/."
