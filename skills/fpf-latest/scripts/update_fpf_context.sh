...
AUTO_STATE_FILE="${FPF_REFRESH_AUTO_STATE_FILE:-$HOME/.local/state/codex-fpf/latest.env}"
...
read_state_value() {
  local key="$1"
  local file
  for file in "$STATE_FILE" "$AUTO_STATE_FILE"; do
    [ -f "$file" ] || continue
    awk -F= -v key="$key" '
      $1 == key {
...
FPF_ENV_CHECK_SUMMARY=The fpf-latest environment check script is missing or not readable.
FPF_ENV_CHECK_ACTION=Restore scripts/check_fpf_environment.sh in the fpf-latest skill.
...
now_epoch="$(date +%s)"
last_attempt_epoch="$(read_state_value LAST_REFRESH_ATTEMPT_EPOCH 2>/dev/null || true)"
if ! is_uint "$last_attempt_epoch"; then
  last_attempt_epoch=""
fi
